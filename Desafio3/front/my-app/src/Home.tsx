...
export default function Home() {
    ...
    const { patientCpf, patientName, birthDate, patientPhone, doctorSpecialisation, doctor_id, data_appointment } = formValues;
    ...
    async function Appointment() {
        const returnAuthenticate = await newAppointment(patientCpf, patientName, birthDate, patientPhone, doctorSpecialisation, doctor_id, data_appointment);
    ...
    return <>
        <Modal isOpen={appointmentVisible} onClose={() => setAppointmentVisible(false)} initialFocusRef={initialRef} finalFocusRef={finalRef}>
            <Modal.Content>
                <Modal.CloseButton />
                ...
                <Modal.Body>
                    <FormControl>
                        ...
                        <FormControl.Label>Data de nascimento do Paciente</FormControl.Label>
                        <Input ref={initialRef}
                            value={birthDate}
                            onChangeText={text => handleChange('birthDate', text)}
                        />

                        <FormControl.Label>Número de celular do Paciente</FormControl.Label>
                        <Input ref={initialRef}
                            value={patientPhone}
                        ...
                        <Input ref={initialRef}
                            ...
                            onChangeText={text => handleChange('doctorSpecialisation', text)}
                        />
                ...
                <Modal.Footer>
                    <Button.Group space={2}>
                        <Button variant='ghost' colorScheme='blueGray' onPress={() => { setAppointmentVisible(false); }}>
                            Cancel
                        </Button>
                        <Button onPress={() => Appointment()}>
                            Salve
                        </Button>
                    </Button.Group>
                </Modal.Footer>
